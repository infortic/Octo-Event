...
import com.octoevents.repository.IssuesEventsRepository;
...
public class IssueEventEndPontTest {
	...
	private void testGetResponseDTOReturnStatusCode200() {
		...
		ResponseEntity<String> response = this.restTemplet.getForEntity("/issues/" + eventTemp.getIssuesId() + "/events",
				String.class);
	...
	public void testPostResponseStatusCode200() {
		...
		ResponseEntity<String> response = this.restTemplet.postForEntity("/events", eventTemp, String.class);
		Assertions.assertThat(response.getStatusCodeValue()).isEqualTo(200);
	}
	...
	public void testPostResponseStatusCode404() {
		...
		eventTemp.setIssue(issueTemp);
		BDDMockito.when(this.issuesEventsRepository.save(eventTemp)).thenReturn(eventTemp);
		ResponseEntity<String> response = this.restTemplet.postForEntity("/evenfts", eventTemp, String.class);
